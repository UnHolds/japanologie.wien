...
export default function HonBun({
  honbun,
  hide_furigana,
  hide_translation,
  className,
    ...
    <div className={`flex justify-center ${className}`}>
      <div className="flex flex-col gap-3">
        <div className="text-4xl font-bold mb-8 text-center">
          {`[${honbun.lesson}.${honbun.honbun}] - `}{" "}
          {format_furiana(honbun.title, _hideFurigana)}
        </div>
        {honbun.story.map((sl, i) => {
          if ((sl as StoryLocation).location) {
            return (
              <HonBunLocation
                key={i}
                location={sl as StoryLocation}
                hide_furigana={_hideFurigana}
                hide_translation={_hideTranslation}
              />
            );
          } else {
            //shoule be storyline
