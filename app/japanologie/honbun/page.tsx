  ...
  return (
    <div className="pt-8 h-full">
      <div className="text-6xl font-bold text-center mb-10">
        {format_furiana("{本|ほん}{文|ぶん}", false)} - HonBun
      </div>
      <div className="text-center mb-10">
        This page needs a lot of help, pls submit the missing honbuns
      </div>
      <div className="flex justify-center gap-6 mb-5">
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="hide_furigana" className="text-xl">
            Hide Furigana
          </label>
        </div>
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="hide_meaning" className="text-xl">
            Hide Translation
          </label>
        </div>
      </div>
        ...
        {honbuns.map((h, i) => {
          return (
            <div className="w-1/2" key={i}>
              <button
                className="bg-blue-500 p-2 rounded w-full flex items-center justify-between"
                onClick={() => {
                  setToShowHonbunId(i == toShowHonbunId ? -1 : i);
                }}
              >
                {toShowHonbunId == i ? (
                  <FontAwesomeIcon
                ...
                <div className="text-2xl ">
                  [{h.lesson}.{h.honbun}] -{" "}
                  {format_furiana(h.title, hideFurigana)}{" "}
                  {hideTranslation ? "" : `(${h.title_translation})`}
                </div>
                <div />
              </button>
              {toShowHonbunId == i && (
