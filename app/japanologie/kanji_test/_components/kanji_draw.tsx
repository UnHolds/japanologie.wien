      ...
      canvas.addEventListener("mousemove", (e) => {
        ...
        pos.current = {
          x:
            e.clientX -
            canvas.offsetLeft -
            (canvas.parentElement?.offsetLeft || 0),
          y:
            e.clientY -
            canvas.offsetTop -
            (canvas.parentElement?.offsetTop || 0) +
            window.scrollY,
        };
  ...
  return (
    ...
    <div className="size-fit">
      ...
      <div className="relative border-3 size-fit">
        <canvas
          ref={canvasRef}
          className={`z-10 absolute top-0 ${show ? "opacity-80" : "opacity-100"}`}
          width={hw.w * scale}
          height={hw.h * scale}
        />
        <Image
          ...
          width={hw.w * scale}
          height={hw.h * scale}
          className={`dark:invert ${show ? "opacity-100" : "opacity-0"}`}
        />
      </div>

      <div className="flex justify-around mt-3 gap-2">
        <button
          className="bg-sky-700 p-2 rounded md:text-xl text-lg font-bold items-center w-10"
          onClick={() =>
            setScale(
              window.innerWidth >= hw.w * (scale + 0.1) ? scale + 0.1 : scale,
            )
          }
        >
          +
        ...
        <button
          className="bg-sky-700 p-2 rounded md:text-xl text-lg font-bold items-center"
