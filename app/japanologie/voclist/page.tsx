      ...
      <div className="flex justify-center gap-6 mb-5">
        <div className="flex items-center gap-2">
          <input
            ...
            name="hide_kana"
            className="w-5 h-5"
            checked={hideKanaKanji}
            onChange={() => {
              setHideKanaKanji(!hideKanaKanji);
            }}
          />
          <label htmlFor="hide_kana" className="text-xl">
            Hide Kana / Kanji
          </label>
        </div>
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="hide_kanji" className="text-xl">
            Hide Meaning
          </label>
        </div>
      </div>
        ...
        <div className="h-full flex flex-col gap-2 xl:w-1/3 lg:w-2/3">
          {vocs.map((v, i) => {
            ...
            if (curr_lesson != v.lesson) {
              ...
              elems.push(
                <div key={`${i}-l`} className="text-center font-bold text-4xl">
                  {curr_lesson} 課
                </div>,
              );
