...
export async function get_assignments(): Promise<Assignment[]> {
  ...
  return (
    await get_data<WaniKaniObject<[WaniKaniObject<Assignment>]>>(url)
  ).data.map((d) => d.data);
}

export async function get_subject_with_id<T extends SubjectBase>(
  id: number,
): Promise<T> {
...
export async function get_subject_with_assignment(
