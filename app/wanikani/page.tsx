...
export default function WaniKani() {
  ...
  useEffect(() => {
    get_assignments().then(r => _setAssignments(r)).catch(() => _setAssignments([]));
    ...
    <div className="h-full">
      <div className="flex h-full justify-center items-center">
        <div>
          ...
          { _apiKeyExists == false && (<h3 className="text-red-500 text-center text-bold text-xl">API-KEY is missing, please add it in the settings</h3>)}
          <div className="my-15 flex gap-5 md:flex-row flex-col mx-6">
            <Link className="flex bg-sky-700 p-4 rounded md:text-4xl text-2xl font-bold items-center" href="/404">Reviews ({_assignments.length})</Link>
          </div>
        </div>
      </div>
    </div>
